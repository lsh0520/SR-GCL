...
class MoleculeDatasetTransAug(InMemoryDataset):
    def __init__(self,
                 root,
                 # data = None,
                 # slices = None,
        ...
        super(MoleculeDatasetTransAug, self).__init__(root, transform, pre_transform, pre_filter)
        self.transform, self.pre_transform, self.pre_filter = transform, pre_transform, pre_filter
        ...
        self.edge_score = torch.zeros([self.data['edge_index'].size(1)], dtype=torch.half)
    ...
    def get(self, idx):
        ...
        if self.aug == 'dropN':
            data = drop_nodes(data, self.aug_ratio)
        elif self.aug == 'dropN_attn':
            nodes_score = self.node_score[self.slices['x'][idx]:self.slices['x'][idx + 1]]
            data = drop_nodes_prob(data, self.aug_ratio, nodes_score)
        elif self.aug == 'dropE_attn':
            edge_score = self.edge_score[self.slices['edge_index'][idx]:self.slices['edge_index'][idx + 1]]
            data = drop_edges_prob(data, self.aug_ratio, edge_score)
        elif self.aug == 'permE':
            data = permute_edges(data, self.aug_ratio)
        elif self.aug == 'maskN':
            data = mask_nodes(data, self.aug_ratio)
            ...
            data = subgraph(data, self.aug_ratio)
        elif self.aug == 'subgraph_attn':
            nodes_score = self.node_score[self.slices['x'][idx]:self.slices['x'][idx + 1]]
            data = subgraph_prob(data, self.aug_ratio, nodes_score)
        elif self.aug == 'random':
            nodes_score = self.node_score[self.slices['x'][idx]:self.slices['x'][idx + 1]]
            n = np.random.randint(2)
            if n == 0:
...
def drop_nodes_prob(data, aug_ratio, node_score):
    ...
    node_prob = F.softmax(node_score.float(), dim=-1).float()
    node_prob += 0.001
...
def drop_edges_prob(data, aug_ratio, edge_score):
    node_num, _ = data.x.size()
    _, edge_num = data.edge_index.size()
    edge_num = int(edge_num/2)
    drop_num = int(edge_num * aug_ratio)
    edge_index = data.edge_index.numpy()

    edge_prob = F.softmax(edge_score[::2].float() + edge_score[1::2].float(), dim=-1).float()
    edge_prob += 0.001
    edge_prob = np.array(edge_prob)
    edge_prob /= edge_prob.sum()

    idx_nondrop = np.random.choice(edge_num, edge_num - drop_num, replace=False, p=edge_prob)
    idx_nondrop = np.append(2 * idx_nondrop, 2 * idx_nondrop + 1)
    idx_nondrop.sort()

    edge_index = torch.tensor(edge_index[:, idx_nondrop])
    edge_attr = data.edge_attr[idx_nondrop, :]
    edge_index, edge_attr, mask = remove_isolated_nodes(edge_index, edge_attr=edge_attr, num_nodes=node_num)
    data.x = data.x[mask, :]
